...
class BaseFeeStructure(models.Model):
    ...
    class_from = models.ForeignKey(StudentClass, on_delete=models.CASCADE, related_name='fee_structures_from', null=True, blank=True)
    class_to = models.ForeignKey(StudentClass, on_delete=models.CASCADE, related_name='fee_structures_to', null=True, blank=True)
    
    fee_head = models.ForeignKey(FeeHead, on_delete=models.CASCADE)
    total_amount = models.DecimalField(max_digits=10, decimal_places=2)
    is_new_student_only = models.BooleanField(default=False)

    def __str__(self):
        from_name = self.class_from.name if self.class_from else "Khaali"
        to_name = self.class_to.name if self.class_to else "Khaali"
        return f"{from_name} to {to_name} - {self.fee_head.name}"
...
class StudentFeeAllocation(models.Model):
    ...
    academic_year = models.ForeignKey(AcademicSession, on_delete=models.CASCADE)
    admission_type = models.CharField(max_length=10, choices=ADMISSION_TYPE_CHOICES, default='OLD')
